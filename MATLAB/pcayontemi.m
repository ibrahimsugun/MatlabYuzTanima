...
D = dir(datapath); 
imgcount = 0; %resim uzunlugunu sifir yap.
for i=1 : size(D,1) % D degiskeninin satir sayisi kadar don ve karsilastirmaa yap
    if not(strcmp(D(i).name,'.')|strcmp(D(i).name,'..')|strcmp(D(i).name,'Thumbs.db'))
        imgcount = imgcount + 1; %veritabanindaki resimleri 1 arttirarak karsilastirma yapiyor.
    end
end
X = [];
for i = 1 : imgcount %veri tabanindaki resim sayisi kadar don.
...
m = mean(X,2); % Ortalama yuz goruntusu  bulunuyor.
